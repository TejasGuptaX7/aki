...
export default function AgentChatPage({ params }: { params: Promise<{ agentId: string }> }) {
  ...
  // Per-agent isolated thread state, keyed by agentId so switching agents
  ...
  // If agentId is bogus (e.g. archived), bounce to /agents once the list is loaded
  React.useEffect(() => {
    ...
    if (!hit) {
      router.replace(`/chat/${agents[0].id}`);
      return;
    }
  ...
  return (
    <AppShell>
      <div style={{ display: "flex", flexDirection: "column", height: "100vh" }}>
        <ChatHeader detail={detail} detailErr={detailErr} agentId={agentId} onClear={() => {
          setHistory([]); setTools([]); setStreaming(""); setErr(null);
        }}/>
          ...
          <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 32px" }}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-end" }}>
              <textarea
                ...
                placeholder={`Ask ${detail?.name ?? "Aki"} to do something with your connected tools…`}
                disabled={pending}
                rows={2}
                style={{
                  flex: 1, resize: "none",
...
function ChatHeader({ detail, detailErr, agentId, onClear }: {
  detail: AgentDetail | null;
  detailErr: string | null;
  agentId: string;
  onClear: () => void;
}) {
  const brief = detail?.system_prompt ?? "";
  const preview = brief.split("\n").find((l) => l.trim().length) ?? "";
  return (
    <div style={{
      padding: "20px 56px", borderBottom: `1px solid ${theme.hair}`,
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        ...
        }}>chat · {detail?.slug ?? "…"}</div>
        <div style={{
          fontFamily: theme.display, fontSize: 22, fontWeight: 600,
          letterSpacing: "-0.015em",
        }}>{detail?.name ?? (detailErr ? "unknown agent" : "loading…")}</div>
        {preview && (
          <div style={{
            marginTop: 4, fontFamily: theme.body, fontSize: 13,
      ...
      <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
        <Link href={`/agents/${agentId}`} style={pillBtn}>Edit brief</Link>
        <button onClick={onClear} style={pillBtn}>New thread</button>
      </div>
    </div>
  );
}
...
function StatusBar({ stage, elapsed, activeTool, toolCount }: {
  stage: Stage; elapsed: number; activeTool?: ToolEvent; toolCount: number;
}) {
  ...
  return (
    <div style={{
      ...
      <Pulse/>
      <div style={{ flex: 1 }}>
        <div style={{ fontFamily: theme.body, fontSize: 14, fontWeight: 500, color: theme.ink }}>{headline}</div>
        {sub && (
          <div style={{
            marginTop: 4, fontFamily: theme.mono, fontSize: 11,
            color: theme.inkDim, wordBreak: "break-all",
          }}>{sub}</div>
        )}
      </div>
      <div style={{
        fontFamily: theme.mono, fontSize: 11, color: theme.inkFaint, letterSpacing: "0.18em",
      }}>{elapsed}s</div>
    </div>
  );
}
...
function EmptyChat({ agentName, onPick }: { agentName: string; onPick: (text: string) => void }) {
  const examples = [
  ...
  return (
    <div style={{ paddingTop: 80, textAlign: "center" }}>
      <div style={{
        fontFamily: theme.display, fontStyle: "italic", fontSize: 36,
        fontWeight: 500, letterSpacing: "-0.02em", color: theme.inkLede, marginBottom: 32,
      }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, maxWidth: 560, margin: "0 auto" }}>
        {examples.map((e) => (
          <button key={e} onClick={() => onPick(e)} style={{
            ...
            border: `1px dashed ${theme.hair}`, color: theme.inkDim,
    ...
    <div style={{
      ...
      {rows.map((t) => (
        <div key={t.id} style={{
          ...
          <span style={{
            color: t.status === "completed" ? theme.accent : theme.inkDim,
